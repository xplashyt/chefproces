 ...
 * mostrarle a quien está pagando. Mismo patrón que lib/payment-options.ts y
 * lib/wompi-endpoint.ts: módulo isomorfo, sin dependencias.
 *
 ...
 *    vencida"...). Wompi no publica una lista cerrada de códigos para esto
 *    —es texto que viene del banco emisor—, así que esto es reconocimiento
 *    de patrones sobre ese texto, con un mensaje de respaldo que NUNCA oculta
 *    el motivo real que dio el banco cuando no reconocemos el patrón.
 *
 * 2. FALLA DE LA PASARELA (`clasificarErrorGateway`): la transacción ni
 ...
 *    respuesta que no es JSON) es la señal que tenemos disponible para "esta
 *    IP/red quedó bloqueada por seguridad" —Wompi no manda un mensaje literal
 *    de "IP baneada", pero esa es la causa real detrás de ese código.
 */
...
export function clasificarRechazo(statusMessage: string | null | undefined): ErrorPago {
  ...
  if (razon) {
    ...
    return {
      ...
      consejo: 'Contacta a tu banco, prueba con otra tarjeta o escríbenos.',
    };
  }
  ...
  return {
    ...
    consejo: 'Prueba con otra tarjeta, revisa tu cupo o escríbenos.',
  };
}

/** Lo que la capa de red SÍ puede observar con certeza sobre una falla de la pasarela. */
export interface FallaGateway {
 ...
 */
export function clasificarErrorGateway(falla: FallaGateway): ErrorPago {
  if (falla.redCaida) {
    return {
      codigo: 'SIN_CONEXION',
